...
Kickstart automatically installs these official plugins on every session start:
...
<!-- NOTE: This table is auto-generated by the auto-docs hook and focuses on structural info such as triggers and jobs. The Workflow column always uses the workflow filename in backticks. Do not edit rows here manually; change workflow files instead and update nearby docs for behavior/setup details. -->
<!-- kickstart:workflows:start -->
| Workflow | Description |
|----------|-------------|
| `address-pr-comments.yml` | Triggers: pull_request_review_comment; Jobs: address-comment |
<!-- kickstart:workflows:end -->

Some workflows (such as `address-pr-comments.yml`) require additional configuration (for example, API keys or repository secrets); see [Anthropic API Key Setup](#anthropic-api-key-setup) and any other workflow-specific setup sections below for configuration details.
