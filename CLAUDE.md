...
| `auto-install-plugins` | SessionStart | Installs companion plugins automatically |
...
Some workflows (such as `address-pr-comments.yml`) require additional configuration (for example, API keys or repository secrets); see the dedicated workflow sections below (for example, "Auto-Address PR Comments") for setup and behavior details.
